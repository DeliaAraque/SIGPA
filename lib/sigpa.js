...
function embem(ruta, obj, parametros)
{
	loading(true);
	...
	if(typeof parametros != "string")
		parametros = new FormData(parametros);
...
function formularios()
{
	...
	{
		var role = $(f[i]).attr("role");

		if(role == "form")
			$(f[i]).attr("onSubmit", "return sendForm(this)");

		else if(role == "search")
		{
			var ruta = $(f[i]).attr("action");
			var obj = $(f[i]).attr("data-obj") || "#page-wrapper";

			var fn =  "embem('" + ruta + "', '" + obj + "', this);";

			$(f[i]).attr("onSubmit", fn + " return false");
		}

		$(f[i]).attr("autocomplete", "off");
	}
}
...
function sendReq(ruta, parametros)
{
	loading(true);
